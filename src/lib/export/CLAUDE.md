...
- **Embedded fonts** (`applyEmbeddedFontsOdf`, `applyEmbeddedFontsDocx`): the faces the opened file carried (`fonts/embeddedFonts.ts`) are written back, or a document in a font nobody has installed would lose its look on the next machine. ODF stores plain TTFs under `Fonts/` with a manifest entry each and one `<style:font-face>` per family whose `<svg:font-face-src>` names every cut (`loext:font-weight`/`-style`) — merged **into** the declaration odf-kit already wrote for the name, since a second one with the same `style:name` is invalid. Word obfuscates each part with a key of its own, which is the same XOR the importer undoes, so each cut gets a fresh GUID, a `word/fonts/font{n}.odttf`, a relationship in `word/_rels/fontTable.xml.rels` and its `<w:embed…>` inside the family's `<w:font>` in that element's schema order (Regular, Bold, Italic, BoldItalic). The `docx` package can embed too, but writes `w:embedRegular` only.
...
- **`mergeListItemBlocks`/`applyListItemBlocks`** — odf-kit writes a list item's **first paragraph only**, so the item's further blocks ride it `SEG`-separated and the pass splits them back out inside the same `<text:list-item>`. They are re-emitted at the item's end (after a nested list, which is where they usually belong) wearing the item's own paragraph style — or a minted `LX#` under it where the extra carries alignment/spacing/line-height/box of its own (collected from the tree *before* `mergeListItemBlocks`, which is what folded them in). A heading extra goes back out as `<text:h>` under its level's style, which is what ODF holds in a list item. DOCX needs none of it: `listToParagraphs` writes the extras as unnumbered paragraphs at the item's indent, which is Word's own shape.
...
- **`applyListItemStyles`** — per-item alignment/spacing/line-height for top-level lists (odf-kit's ListBuilder has no per-item options) via minted automatic styles. It runs **after** `applyListItemBlocks`, so an extra block's own style parents to the item's list style rather than to the `LP#` minted here. Both take their descriptors in list-item order, and `collectListItemStyles` therefore skips tables: a cell's list is still `SEG`-segmented text at this point (`applyCellBlocks` rebuilds it later), so counting its items would shift every body item's style. A **page break** before an item rides here too: `replacePageBreaks` is top-level only (its PGB sentinel would corrupt the `SEG` rebuild), and LibreOffice keeps `fo:break-before` on a list item's paragraph (probed).
...
- **A field's cached run carries its formatting.** `SimpleField` writes a bare `TextRun` for the cached value, so a `STYLEREF` running head or an auto date field lost its own marks; `StyledField` pushes the run with them instead.
- **A sentinel pass must cover the note parts.** A footnote's or endnote's text is its own package part, so a post-pack pass that only rewrote `word/document.xml` left the sentinel standing there — a formula in a note read back as its index, a ruby as its base. `NOTE_BEARING_PARTS` is the list the formula and ruby passes walk (`docx.ts`).
- **Vertical margins travel in pairs.** LibreOffice reads a style declaring only one of `fo:margin-top`/`fo:margin-bottom` as resetting the other to the *default* style's, not the parent's (probed: re-saving a `Heading_20_2` automatic style with only `fo:margin-top` comes back with `fo:margin-bottom="0cm"`). So `pairMargins` writes the named style's partner out beside a block whose spacing is direct formatting — top-level and in a table cell, where a heading keeps its level's margins — and `ownStyleAttrs` does the same for a named style — otherwise setting a heading's space-before silently zeroes its space-after.
