// German catalog. Typed as `Messages`, so a missing or mistyped key — or an
// interpolation function with the wrong signature — is a svelte-check error.

import type { Messages } from './en';

const de: Messages = {
  common: {
    ok: 'OK',
    cancel: 'Abbrechen',
    close: 'Schließen',
    apply: 'Übernehmen',
    insert: 'Einfügen',
    add: 'Hinzufügen',
    remove: 'Entfernen',
  },
  docProps: {
    title: 'Dokumenteigenschaften',
    docTitle: 'Titel',
    subject: 'Thema',
    author: 'Autor',
    keywords: 'Schlüsselwörter',
    keywordsHint: 'durch Komma getrennt',
    description: 'Kommentare',
    clear: 'Alle löschen',
  },
  autoCorrect: {
    title: 'AutoKorrektur w\u00E4hrend der Eingabe',
    quotes: 'Typografische Anf\u00FChrungszeichen',
    dashes: 'Gedankenstriche ersetzen',
    replacements: 'Ersetzungstabelle',
    capitalize: 'Jeden Satz mit einem Gro\u00DFbuchstaben beginnen',
    twoInitials: 'ZWei GRo\u00DFbuchstaben am Wortanfang korrigieren',
    urls: 'URL-Erkennung',
    reset: 'Standard wiederherstellen',
    examples: {
      quotes: '"a" \u2192 \u201Ea\u201C',
      dashes: 'A - B \u2192 A \u2013 B',
      replacements: '--> \u2192 \u2192, (C) \u2192 \u00A9',
      capitalize: '',
      twoInitials: 'WOrt \u2192 Wort',
      urls: '',
    },
  },
  ruby: {
    title: 'Ruby-Text',
    base: 'Basistext',
    reading: 'Lesung',
  },
  autoText: {
    title: 'AutoText',
    empty: 'Noch keine Einträge. Text markieren und unten hinzufügen.',
    newEntry: 'Neu aus Auswahl',
    name: 'Name',
    shortcut: 'Kürzel',
    insertEntry: 'Diesen Eintrag einfügen',
    needsSelection: 'Zuerst den zu sichernden Text markieren',
    f3Hint: 'Kürzel tippen und F3 drücken, um den Eintrag einzufügen.',
  },
  wordCompletion: {
    title: 'Wortergänzung',
    enabled: 'Wörter während der Eingabe ergänzen',
    hint: 'Eingabetaste übernimmt, Esc verwirft',
    appendSpace: 'Leerzeichen anhängen',
    minLength: 'Mindestwortlänge',
    clear: 'Gesammelte Wörter löschen',
    collected: (n) => `${n} ${n === 1 ? 'Wort' : 'Wörter'} gesammelt`,
  },
  navigator: {
    title: 'Navigator',
    empty: 'Noch keine Überschriften. Gib einem Absatz eine Überschriftenvorlage, dann erscheint er hier.',
    untitled: '(ohne Titel)',
    moveUp: 'Kapitel nach oben verschieben',
    moveDown: 'Kapitel nach unten verschieben',
    promote: 'Ebene höherstufen',
    demote: 'Ebene tieferstufen',
    tables: 'Tabellen',
    images: 'Bilder',
    bookmarks: 'Lesezeichen',
    tableN: (n) => `Tabelle ${n}`,
    imageN: (n) => `Bild ${n}`,
  },
  comments: {
    title: 'Kommentare',
    onPage: (n: number) => `Seite ${n}`,
    empty: 'Noch keine Kommentare. Text markieren und einen hinzufügen.',
    more: 'Mehr',
    reply: 'Antworten',
    edit: 'Bearbeiten',
    resolve: 'Erledigt',
    reopen: 'Wieder öffnen',
    prompt: 'Kommentar zum markierten Text:',
    needsSelection: 'Zuerst den zu kommentierenden Text markieren',
    showPane: 'Kommentare anzeigen',
    newComment: 'Neuer Kommentar',
  },
  appearance: {
    language: 'Sprache',
    title: 'Darstellung',
    light: 'Hell',
    dark: 'Dunkel',
    allBlack: 'Komplett Dunkel',
    auto: 'Automatisch',
    allBlackHint: 'Schriftfarben werden weiß erzwungen',
  },
  shortcut: {
    ctrl: 'Strg',
    shift: 'Umschalt',
    alt: 'Alt',
  },
  align: {
    title: 'Textausrichtung',
    section: 'Ausrichtung',
    left: 'Links',
    center: 'Zentriert',
    right: 'Rechts',
    justify: 'Blocksatz',
    alignTo: (label: string) => `${label} ausrichten`,
  },
  toolbar: {
    bold: 'Fett',
    italic: 'Kursiv',
    underline: 'Unterstrichen',
    strikethrough: 'Durchgestrichen',
    heading1: 'Überschrift 1',
    heading2: 'Überschrift 2',
    heading3: 'Überschrift 3',
    heading4: 'Überschrift 4',
    heading5: 'Überschrift 5',
    heading6: 'Überschrift 6',
    styles: {
      title: 'Formatvorlagen',
      default: 'Standard',
      docTitle: 'Titel',
      subtitle: 'Untertitel',
      quote: 'Zitat',
    },
    bulletList: 'Aufzählung',
    bulletSymbol: 'Aufzählungszeichen',
    bulletDefault: 'Standard (nach Ebene)',
    orderedList: 'Nummerierte Liste',
    numberingStyle: 'Nummerierungsstil',
    numbering: 'Nummerierung',
    bullets: {
      '•': 'Punkt',
      '◦': 'Kreis',
      '▪': 'Quadrat',
      '❖': 'Raute',
      '➢': 'Pfeilspitze',
      '⇨': 'Pfeil',
      '✓': 'Haken',
      '–': 'Strich',
      '>': 'Winkel',
    },
  },
  styles: {
    title: 'Formatvorlagen',
    manage: 'Vorlagen verwalten…',
    showAll: 'Alle Formatvorlagen anzeigen',
    characterStyles: 'Zeichenvorlagen',
    paragraphStyles: 'Absatzvorlagen',
    newStyle: 'Neu',
    custom: 'eigen',
    inherited: 'geerbt',
    abstract: 'nur Elternstil',
    parent: 'Erbt von',
    font: 'Schriftart',
    size: 'Größe (pt)',
    color: 'Farbe',
    bold: 'Fett',
    italic: 'Kursiv',
    underline: 'Unterstrichen',
    strike: 'Durchgestrichen',
    alignment: 'Ausrichtung',
    spaceBefore: 'Abstand oben (pt)',
    spaceAfter: 'Abstand unten (pt)',
    indent: 'Einzug (cm)',
    newFromSelection: 'Neu aus Auswahl',
    updateFromSelection: 'Aus Auswahl aktualisieren',
    rename: 'Umbenennen…',
    reset: 'Auf Standard zurücksetzen',
    newName: 'Vorlage',
    // Short forms for the manager's tab strip; the long names above still label
    // the gallery sections.
    tabs: { paragraph: 'Absatz', character: 'Zeichen', table: 'Tabelle', list: 'Liste' },
    tableStyles: 'Tabellenvorlagen',
    listStyles: 'Listenformatvorlagen',
    manageListStyles: 'Listenformatvorlagen verwalten…',
    level: 'Ebene',
    levelKind: { bullet: 'Aufzählungszeichen', number: 'Nummerierung' },
    bulletChar: 'Aufzählungszeichen',
    numbering: 'Nummerierung',
    indentStep: 'Einzugsschritt (cm)',
    markerRight: 'Nummer rechtsbündig',
    startAt: 'Beginnen mit',
    multilevelAll: 'Mehrstufige Nummerierung (1, 1.1, 1.2.1)',
    addLevel: 'Ebene hinzufügen',
    removeLevel: 'Letzte Ebene entfernen',
    region: 'Bereich',
    fill: 'Füllung',
    outerBorder: 'Außenrahmen',
    rowLines: 'Zeilenlinien',
    columnLines: 'Spaltenlinien',
    noBorder: 'Keiner',
    regions: {
      wholeTable: 'Ganze Tabelle',
      headerRow: 'Kopfzeile',
      lastRow: 'Letzte Zeile',
      firstColumn: 'Erste Spalte',
      lastColumn: 'Letzte Spalte',
      bandedRow: 'Streifenzeilen',
      bandedColumn: 'Streifenspalten',
    } as Record<string, string>,
  },
  history: {
    undo: 'Rückgängig',
    redo: 'Wiederholen',
    heading: (dir) => (dir === 'undo' ? 'Rückgängig-Verlauf' : 'Wiederherstellungsverlauf'),
    revertCount: (dir, n) =>
      `${n} ${n === 1 ? 'Aktion' : 'Aktionen'} ${dir === 'undo' ? 'rückgängig machen' : 'wiederholen'}`,
    actionCount: (n) => `${n} ${n === 1 ? 'Aktion' : 'Aktionen'}`,
  },
  toolbarExpanded: {
    fontName: 'Schriftart',
    recent: 'Zuletzt',
    webSafe: 'Web-sicher',
    allFonts: 'Alle Schriften',
    loadAllFonts: 'Alle installierten Schriften laden',
    fontSize: 'Schriftgröße',
    fontSizeList: 'Schriftgrößenliste',
    growFont: 'Schrift vergrößern',
    shrinkFont: 'Schrift verkleinern',
    fontColor: 'Schriftfarbe',
    chooseFontColor: 'Schriftfarbe wählen',
    highlightColor: 'Hervorhebungsfarbe',
    chooseHighlightColor: 'Hervorhebungsfarbe wählen',
    noColor: 'Keine Farbe',
    paragraphShading: 'Absatzhintergrund',
    chooseParagraphShading: 'Absatzhintergrund wählen',
    paragraphBorders: 'Absatzränder',
    superscript: 'Hochgestellt',
    subscript: 'Tiefgestellt',
    decreaseIndent: 'Einzug verkleinern',
    increaseIndent: 'Einzug vergrößern',
    lineParagraphSpacing: 'Zeilen- & Absatzabstand',
    lineSpacing: 'Zeilenabstand',
    lineSingle: 'Einfach',
    lineDouble: 'Doppelt',
    spaceBefore: 'Abstand vor Absatz (pt)',
    spaceAfter: 'Abstand nach Absatz (pt)',
    spaceBeforePresets: 'Voreinstellungen Abstand davor',
    spaceAfterPresets: 'Voreinstellungen Abstand danach',
    spacingDefault: 'Standard',
    pt: (v) => `${v} pt`,
    pageLayoutMargins: 'Seitenlayout & Ränder',
    layout: 'Layout',
    pageFormat: 'Seitenformat',
    pageFormats: {
      A3: 'A3', A4: 'A4', A5: 'A5', A6: 'A6',
      isoB4: 'B4 (ISO)', isoB5: 'B5 (ISO)', isoB6: 'B6 (ISO)',
      jisB4: 'B4 (JIS)', jisB5: 'B5 (JIS)',
      letter: 'Letter', legal: 'Legal', tabloid: 'Tabloid',
      executive: 'Executive', folio: 'Folio', statement: 'Statement',
    },
    orientation: 'Ausrichtung',
    portrait: 'Hochformat',
    landscape: 'Querformat',
    pageMargins: 'Seitenränder (cm)',
    margins: { top: 'Oben', bottom: 'Unten', left: 'Links', right: 'Rechts', inner: 'Innen', outer: 'Außen' },
    marginField: (label) => `Rand ${label.toLowerCase()} (cm)`,
    increaseMargin: (label) => `Rand ${label.toLowerCase()} vergrößern`,
    decreaseMargin: (label) => `Rand ${label.toLowerCase()} verkleinern`,
    headerFooter: 'Kopf- & Fußzeile',
    editHeader: 'Kopfzeile bearbeiten',
    editFooter: 'Fußzeile bearbeiten',
    mirrorMargins: 'Gespiegelte Ränder',
    mirrorMarginsHint: 'Linken und rechten Rand auf geraden Seiten tauschen, damit der innere zur Bindung zeigt',
    differentFirstPage: 'Erste Seite anders',
    differentFirstPageHint: 'Der ersten Seite eine eigene Kopf- und Fußzeile geben',
    differentOddEven: 'Gerade & ungerade Seiten anders',
    differentOddEvenHint: 'Geraden Seiten eine eigene Kopf- und Fußzeile geben',
    position: 'Position (cm)',
    hfDist: { header: 'Kopfzeile von oben', footer: 'Fußzeile von unten' },
    hfField: (label) => `${label} (cm)`,
    increaseShort: 'Vergrößern',
    decreaseShort: 'Verkleinern',
    increase: (label) => `${label} vergrößern`,
    decrease: (label) => `${label} verkleinern`,
    formattingMarks: 'Formatierungszeichen',
    insertImage: 'Bild einfügen',
    insertTextBox: 'Textfeld einfügen',
    textBoxNotInHf: 'Textfelder sind in Kopf-/Fußzeilen nicht verfügbar',
    columns: 'Spalten',
    columnsOne: 'Eine Spalte',
    columnsTwo: 'Zwei Spalten',
    columnsThree: 'Drei Spalten',
    columnGap: 'Spaltenabstand (cm)',
    columnsNotInHf: 'Spalten sind in Kopf-/Fußzeilen nicht verfügbar',
    insertToc: 'Inhaltsverzeichnis einfügen',
    tocNotInHf: 'Ein Inhaltsverzeichnis ist in Kopf-/Fußzeilen nicht verfügbar',
    insertLink: 'Link einfügen',
    linkNotInHf: 'Links sind in Kopf-/Fußzeilen nicht verfügbar',
    insertBookmark: 'Lesezeichen',
    bookmarkNeedsSelection: 'Markiere den Text, den das Lesezeichen umfassen soll',
    bookmarkNotInHf: 'Lesezeichen sind in Kopf-/Fußzeilen nicht verfügbar',
    insertCrossRef: 'Querverweis einfügen',
    newComment: 'Neuer Kommentar',
    crossRefNeedsBookmark: 'Erst ein Lesezeichen setzen — ein Querverweis zeigt darauf',
    insertFootnote: 'Fußnote einfügen',
    insertEndnote: 'Endnote einfügen',
    noteNotInHf: 'Notizen sind in Kopf-/Fußzeilen nicht verfügbar',
    clearFormatting: 'Formatierung entfernen',
  },
  image: {
    wrapToolbar: 'Bildumbruch',
    wrapInline: 'Mit Text in Zeile',
    wrapLeft: 'Textumbruch — Bild links',
    wrapRight: 'Textumbruch — Bild rechts',
    wrapTopBottom: 'Oben und unten',
    wrapBehind: 'Hinter den Text',
    wrapFront: 'Vor den Text',
  },
  textBox: {
    toolbar: 'Textfeld-Optionen',
    wrapInline: 'Mit Text in Zeile',
    wrapLeft: 'Textumbruch — Feld links',
    wrapRight: 'Textumbruch — Feld rechts',
    wrapTopBottom: 'Oben und unten',
    wrapBehind: 'Hinter den Text',
    wrapFront: 'Vor den Text',
    alignLeft: 'Linksbündig in der Spalte',
    alignCenter: 'Zentriert in der Spalte',
    alignRight: 'Rechtsbündig in der Spalte',
    shape: 'Form',
    chooseShape: 'Form auswählen',
    shapes: {
      textbox: 'Rechteck',
      roundRect: 'Abgerundetes Rechteck',
      ellipse: 'Ellipse',
      triangle: 'Gleichschenkliges Dreieck',
      rightTriangle: 'Rechtwinkliges Dreieck',
      diamond: 'Raute',
      pentagon: 'Fünfeck',
      hexagon: 'Sechseck',
      star5: 'Fünfzackiger Stern',
      trapezoid: 'Trapez',
      parallelogram: 'Parallelogramm',
      rightArrow: 'Pfeil nach rechts',
      leftArrow: 'Pfeil nach links',
      upArrow: 'Pfeil nach oben',
      downArrow: 'Pfeil nach unten',
      line: 'Linie',
      lineArrow: 'Pfeil',
      lineDoubleArrow: 'Doppelpfeil',
    },
    verticalText: 'Senkrechter Text',
    vAlignTop: 'Text oben',
    vAlignMiddle: 'Text mittig',
    vAlignBottom: 'Text unten',
    fillColor: 'Füllfarbe',
    chooseFillColor: 'Füllfarbe wählen',
    noFill: 'Keine Füllung',
    borderColor: 'Rahmenfarbe',
    chooseBorderColor: 'Rahmenfarbe wählen',
    noBorder: 'Kein Rahmen',
    borderWidth: 'Rahmenbreite',
    pt: (w) => `${w} pt`,
  },
  bookmark: {
    dialogLabel: 'Lesezeichen',
    namePlaceholder: 'Name des Lesezeichens',
    existing: 'In diesem Dokument',
    none: 'Noch keine Lesezeichen',
    crossRefLabel: 'Querverweis einfügen',
    formatText: 'Text des Lesezeichens',
    formatPage: 'Seitenzahl',
    openHint: (mod) => `${mod}+Klick, um zum Lesezeichen zu springen`,
  },
  link: {
    dialogLabel: 'Link einfügen',
    openHint: (mod) => `${mod}+Klick, um den Link zu öffnen`,
  },
  spellPicker: {
    label: 'Rechtschreibsprache',
    noSpellCheck: 'Keine Rechtschreibprüfung',
  },
  view: {
    split: 'Teilen',
    splitTitle: 'Zwei Ansichten dieses Dokuments, getrennt scrollbar',
    pagesAcross: 'Mehrere Seiten',
    pagesAcrossTitle: 'Mehrere Seiten nebeneinander anzeigen',
    pagesCount: (n) => (n === 1 ? 'Eine Seite' : `${n} Seiten`),
  },
  ruler: {
    show: 'Lineal',
    firstLineIndent: 'Erstzeileneinzug',
    leftIndent: 'Linker Einzug',
    rightIndent: 'Rechter Einzug',
    tabType: {
      left: 'Linker Tabstopp',
      center: 'Zentrierter Tabstopp',
      right: 'Rechter Tabstopp',
      decimal: 'Dezimaltabstopp',
    },
  },
  color: {
    automatic: 'Automatisch',
    moreColors: 'Weitere Farben…',
    pickFromPage: 'Farbe von der Seite auswählen',
    chooseColor: 'Farbe wählen',
  },
  table: {
    editing: 'Tabelle bearbeiten',
    insertRowAbove: 'Zeile oberhalb einfügen',
    insertRowBelow: 'Zeile unterhalb einfügen',
    insertColumnLeft: 'Spalte links einfügen',
    insertColumnRight: 'Spalte rechts einfügen',
    deleteRow: 'Zeile löschen',
    deleteColumn: 'Spalte löschen',
    deleteTable: 'Tabelle löschen',
    mergeCells: 'Zellen verbinden',
    splitCells: 'Zellen teilen…',
    splitCellsAria: 'Zellen teilen',
    cellShading: 'Zellenschattierung',
    cellAlignTop: 'Zellinhalt oben ausrichten',
    cellAlignMiddle: 'Zellinhalt vertikal zentrieren',
    cellAlignBottom: 'Zellinhalt unten ausrichten',
    chooseCellColor: 'Zellenfarbe wählen',
    noFill: 'Keine Füllung',
    insertTable: 'Tabelle einfügen',
    selectSize: 'Tabellengröße wählen',
    dimensions: (cols, rows) => `${cols} × ${rows}`,
    byDimension: (c, r) => `${c} mal ${r}`,
    rows: 'Zeilen',
    cols: 'Spalten',
    numberOfRows: 'Anzahl der Zeilen',
    numberOfColumns: 'Anzahl der Spalten',
    insert: 'Einfügen',
    splitDialogLabel: 'Zellen teilen',
    columns: 'Spalten',
    rowsField: 'Zeilen',
    split: 'Teilen',
    sort: 'Sortieren…',
    sortAria: 'Sortieren',
    sortDialogLabel: 'Tabelle sortieren',
    sortBy: 'Sortieren nach',
    sortThenBy: 'Dann nach',
    sortNone: '(keine)',
    sortDirection: 'Reihenfolge',
    sortType: 'Sortierart',
    sortTypeAuto: 'Automatisch',
    sortTypeNumber: 'Numerisch',
    sortTypeText: 'Alphanumerisch',
    sortAscending: 'Aufsteigend',
    sortDescending: 'Absteigend',
    sortHeaderRow: 'Erste Zeile ist eine Kopfzeile',
    numberFormat: 'Zahlenformat',
    numberFormats: {
      general: 'Allgemein', int: '1235', dec2: '1234,57',
      group2: '1.234,57', percent: '123457 %', percent2: '123456,79 %',
      currency: '1.234,57 €', date: '15.03.23',
    } as Record<string, string>,
    formula: 'Formel…',
    formulaAria: 'Formel',
    formulaDialogLabel: 'Tabellenformel',
    formulaField: 'Formel',
    formulaCell: (name: string) => `Zelle ${name}`,
    // LibreOffice' eigener Wortlaut für eine Formel, die es nicht lesen kann.
    formulaError: '** Ausdruck ist fehlerhaft **',
    numberRecognition: 'Zahlenerkennung',
    numberRecognitionHint: 'Eine Zelle, die eine Zahl enthält, als Zahl lesen',
    // No trailing ellipsis here (unlike the other dialog-opening entries): the
    // row truncates, so CSS adds the dots only when the label doesn't fit.
    manageStyles: 'Tabellenvorlagen verwalten',
    tableStyle: 'Tabellenvorlage',
    styleOptions: 'Optionen',
    noTableStyle: 'Keine',
    styleNames: {
      'Simple Grid': 'Gitter',
      'Simple Grid Rows': 'Gitter Zeilen',
      'Simple Grid Columns': 'Gitter Spalten',
      'Simple List Shaded': 'Liste schattiert',
      'Simple List Columns': 'Liste Spalten',
      'Plain Table': 'Schlicht',
      'Box List Blue': 'Kasten blau',
      'Box List Green': 'Kasten grün',
      'Box List Red': 'Kasten rot',
      'Box List Yellow': 'Kasten gelb',
      'Grid Table Accent': 'Gitter Akzent',
      'List Table Accent': 'Liste Akzent',
      Academic: 'Akademisch',
      Financial: 'Finanzen',
    } as Record<string, string>,
  },
  borders: {
    title: 'Rahmen',
    all: 'Alle Rahmen',
    outer: 'Außenrahmen',
    inner: 'Innenrahmen',
    innerH: 'Innerer waagerechter Rahmen',
    innerV: 'Innerer senkrechter Rahmen',
    top: 'Oberer Rahmen',
    bottom: 'Unterer Rahmen',
    left: 'Linker Rahmen',
    right: 'Rechter Rahmen',
    none: 'Kein Rahmen',
    lineWidth: 'Linienbreite',
    lineColor: 'Linienfarbe',
    pt: (w) => `${w}pt`,
  },
  spell: {
    suggestions: 'Rechtschreibvorschläge',
    noSuggestions: 'Keine Vorschläge',
    addToDictionary: 'Zum Wörterbuch hinzufügen',
    ignoreAll: 'Alle ignorieren',
  },
  thesaurus: {
    // "Thesaurus" heißt das Werkzeug in beiden Programmen, sagt aber nicht, was es tut:
    // sichtbar ist überall "Synonyme", der Fachbegriff steht im Tooltip.
    title: 'Synonyme',
    menuItem: 'Synonyme…',
    hint: 'Ein anderes Wort mit gleicher Bedeutung finden (Thesaurus)',
    lookUp: 'Nachschlagen',
    word: 'Wort eingeben…',
    loading: 'Wird geladen…',
    replaceHint: 'Ein Wort anklicken, um es im Dokument zu ersetzen.',
    noSynonyms: 'Keine Synonyme gefunden',
  },
  contextMenu: {
    cut: 'Ausschneiden',
    copy: 'Kopieren',
    paste: 'Einfügen',
    pasteWithoutFormatting: 'Ohne Formatierung einfügen',
    pasteBlocked: 'Dieser Browser erlaubt das Einfügen nur über die Tastatur. Drücke Strg+V oder halte beim Rechtsklick die Umschalttaste, um das Browser-Menü zu nutzen.',
    insertLink: 'Link…',
    editLink: 'Link bearbeiten…',
    removeLink: 'Link entfernen',
    insertBookmark: 'Lesezeichen…',
    insertCrossRef: 'Querverweis…',
    newComment: 'Kommentar…',
    clearFormatting: 'Direkte Formatierung löschen',
  },
  about: {
    label: 'Über EdenText',
    tagline: 'Textverarbeitung — nur eine URL entfernt.',
    points: ['Free · Open Source · Private by Design'],
    version: (v) => `Version ${v}`,
    viewOnGitHub: 'Auf GitHub ansehen',
    licenses: 'Lizenzen Dritter',
  },
  findReplace: {
    dialogLabel: 'Suchen und ersetzen',
    hideReplace: 'Ersetzen ausblenden',
    showReplace: 'Ersetzen anzeigen',
    toggleReplace: 'Ersetzen umschalten',
    findPlaceholder: 'Suchen',
    noResults: 'Keine Treffer',
    matchCase: 'Groß-/Kleinschreibung beachten',
    wholeWord: 'Ganzes Wort',
    regex: 'Regulärer Ausdruck ($1…$9 im Ersetzungstext)',
    format: 'Formatierung suchen und ersetzen',
    formatFind: 'Suchen:',
    formatReplace: 'Ersetzen:',
    font: 'Schrift',
    sizePt: 'pt',
    color: 'Farbe',
    noColor: 'Beliebige Farbe',
    paragraphStyle: 'Absatzvorlage',
    anyStyle: 'Beliebige Vorlage',
    previous: 'Zurück',
    previousAria: 'Vorheriger Treffer',
    next: 'Weiter',
    nextAria: 'Nächster Treffer',
    replacePlaceholder: 'Ersetzen',
    replaceCurrent: 'Aktuellen Treffer ersetzen',
    replaceAllTitle: 'Alle Treffer ersetzen',
    replace: 'Ersetzen',
    all: 'Alle',
  },
  special: {
    insertChar: 'Sonderzeichen einfügen',
    searchPlaceholder: 'Nach Name oder Code suchen…',
    searchAria: 'Sonderzeichen suchen',
    recent: 'Zuletzt',
    noMatch: 'Keine passenden Zeichen',
    categories: {
      Punctuation: 'Interpunktion',
      Currency: 'Währung',
      Math: 'Mathematik',
      Arrows: 'Pfeile',
      Greek: 'Griechisch',
      Latin: 'Lateinisch',
      Symbols: 'Symbole',
    },
  },
  dateTime: {
    insert: 'Datum und Uhrzeit einfügen',
    dateHeading: 'Datum',
    timeHeading: 'Uhrzeit',
    updateAutomatically: 'Automatisch aktualisieren',
    updateHint: 'Beim Öffnen des Dokuments auf das aktuelle Datum/die Uhrzeit aktualisieren',
  },
  formula: {
    insert: 'Formel einfügen',
    title: 'Formel',
    source: 'Formel (LaTeX)',
    displayFormula: 'Abgesetzte Formel (eigene zentrierte Zeile)',
    groups: {
      structures: 'Strukturen',
      operators: 'Operatoren',
      relations: 'Zeichen',
      greek: 'Griechisch',
    },
  },
  hf: {
    addHeaderHint: 'Zum Hinzufügen einer Kopfzeile doppelklicken',
    addFooterHint: 'Zum Hinzufügen einer Fußzeile doppelklicken',
    headerPlaceholder: 'Kopfzeile…',
    footerPlaceholder: 'Fußzeile…',
    headerLabel: 'Kopfzeile',
    footerLabel: 'Fußzeile',
    section: 'Abschnitt',
    firstPageHeader: 'Kopfzeile erste Seite',
    firstPageFooter: 'Fußzeile erste Seite',
    evenPageHeader: 'Kopfzeile gerade Seiten',
    evenPageFooter: 'Fußzeile gerade Seiten',
    insert: 'Einfügen',
    pageNumber: 'Seitenzahl',
    pageNumberTitle: 'Aktuelle Seitenzahl einfügen',
    pageCount: 'Seitenanzahl',
    pageCountTitle: 'Gesamtseitenanzahl einfügen',
    chapter: 'Kapitel',
    chapterTitle: 'Überschrift des auf der Seite laufenden Kapitels einfügen',
    done: 'Fertig',
    doneTitle: 'Bearbeiten der Kopf-/Fußzeile beenden',
  },
  paragraphDialog: {
    title: 'Absatz',
    indentsTab: 'Einzüge und Abstände',
    breaksTab: 'Zeilen- und Seitenumbruch',
    textDirection: 'Textfluss',
    dirPage: 'Wie die Seite',
    dirLtr: 'Von links nach rechts',
    dirRtl: 'Von rechts nach links',
    widowControl: 'Absatzkontrolle',
    keepNext: 'Nicht vom nächsten Absatz trennen',
    keepLines: 'Zeilen nicht trennen',
    hyphenate: 'Automatische Silbentrennung',
    pageBreakBefore: 'Seitenumbruch oberhalb',
    flowHint: 'Diese Optionen bestimmen, wo der Absatz über Seiten getrennt werden darf.',
    tabsButton: 'Tabstopps…',
  },
  tabsDialog: {
    title: 'Tabstopps',
    none: 'Dieser Absatz hat keine Tabstopps',
    position: 'Position',
    noLeader: 'Kein Füllzeichen',
    defaultInterval: 'Standardtabstopps',
  },
  caption: {
    title: 'Beschriftung einfügen',
    category: 'Kategorie',
    categories: { figure: 'Abbildung', table: 'Tabelle' },
    label: 'Bezeichnung',
    separator: 'Trennzeichen',
    text: 'Beschriftung',
    position: 'Position',
    above: 'Über dem Objekt',
    below: 'Unter dem Objekt',
  },
  bibliography: {
    style: 'Zitierstil',
    styleNeedsIndex: 'Zuerst ein Literaturverzeichnis einfügen — es trägt den Stil',
    styles: { key: 'Kurzname [Schlüssel]', numbered: 'Nummeriert [1]', apa: 'APA', mla: 'MLA', chicago: 'Chicago' },
    title: 'Literaturangabe einfügen',
    identifier: 'Kurzname',
    type: 'Typ',
    types: {
      book: 'Buch', article: 'Zeitschriftenartikel', inbook: 'Buchkapitel',
      incollection: 'In einem Sammelband', conference: 'Konferenzbeitrag',
      techreport: 'Bericht', phdthesis: 'Dissertation', mastersthesis: 'Masterarbeit',
      www: 'Webseite', misc: 'Sonstiges',
    },
    fields: {
      author: 'Autor', title: 'Titel', year: 'Jahr', publisher: 'Verlag',
      journal: 'Zeitschrift', pages: 'Seiten', url: 'URL',
    },
  },
  pageDecor: {
    title: 'Seitengestaltung',
    background: 'Hintergrund',
    useBackground: 'Seite einfärben',
    border: 'Umrandung',
    useBorder: 'Seite umrahmen',
    borderWidth: 'Breite (pt)',
    borderPadding: 'Abstand zum Text (cm)',
    watermark: 'Wasserzeichen',
    watermarkText: 'Text',
    watermarkHint: 'z. B. ENTWURF',
    watermarkColor: 'Farbe',
    watermarkAngle: 'Winkel (°)',
    watermarkTransparency: 'Transparenz (%)',
    clear: 'Alles entfernen',
  },
  templates: {
    title: 'Neu aus Vorlage',
    din5008b: {
      name: 'Brief nach DIN 5008 (Form B)',
      description: 'Geschäftsbrief mit vollem Briefkopf: Anschriftfeld fürs Fensterkuvert ab 45 mm, Infoblock, Falzmarken.',
    },
    din5008a: {
      name: 'Brief nach DIN 5008 (Form A)',
      description: 'Geschäftsbrief mit einzeiligem Briefkopf: Anschriftfeld ab 27 mm, Infoblock, Falzmarken — 18 mm mehr Text auf Seite 1.',
    },
    privateLetter: {
      name: 'Privatbrief',
      description: 'Privater Brief auf derselben Geometrie: Anschriftfeld fürs Fensterkuvert, Ort/Datum-Zeile, Falzmarken.',
    },
    letter: {
      companyName: 'Firmenname',
      returnAddress: 'Absender · Straße · PLZ Ort',
      recipientCompany: 'Firma',
      recipientName: 'Vorname Name',
      recipientStreet: 'Straße Hausnummer',
      recipientCity: 'PLZ Ort',
      yourRef: 'Ihr Zeichen: ',
      yourMessage: 'Ihre Nachricht vom: ',
      ourRef: 'Unser Zeichen: ',
      phone: 'Telefon: ',
      email: 'E-Mail: ',
      dateLabel: 'Datum: ',
      reference: 'Zeichen',
      date: 'Datum',
      phoneNumber: 'Nummer',
      emailAddress: 'Adresse',
      place: 'Ort',
      remark: 'Vermerk (z. B. Einschreiben)',
      subject: 'Betreff',
      salutation: 'Anrede',
      bodyText: 'Brieftext',
      closing: 'Mit freundlichen Grüßen',
      signature: 'Vorname Name',
    },
    coverLetter: {
      name: 'Bewerbungsanschreiben',
      description: 'Bewerbung auf der Briefgeometrie: Absenderblock, Betreff mit Stellenbezeichnung, drei Textabsätze, Anlagenvermerk.',
      phone: 'Telefonnummer',
      email: 'E-Mail-Adresse',
      subject: 'Bewerbung als ',
      position: 'Stellenbezeichnung',
      subjectRef: 'Ihre Stellenanzeige vom ',
      intro: 'Einleitung: warum Sie sich bewerben',
      main: 'Hauptteil: Erfahrung und Stärken',
      final: 'Schluss: Verfügbarkeit und Gesprächswunsch',
      enclosuresLabel: 'Anlagen: ',
      enclosures: 'Lebenslauf, Zeugnisse',
    },
    cv: {
      name: 'Lebenslauf',
      description: 'Tabellarischer Lebenslauf: Kontaktzeile, Berufserfahrung, Ausbildung und Kenntnisse im zweispaltigen Tab-Layout, Ort, Datum und Unterschrift.',
      phone: 'Telefonnummer',
      email: 'E-Mail-Adresse',
      experience: 'Berufserfahrung',
      education: 'Ausbildung',
      skills: 'Kenntnisse',
      period: 'MM/JJJJ – MM/JJJJ',
      role: 'Position',
      employer: 'Unternehmen, Ort',
      roleDetail: 'Aufgaben und Erfolge',
      degree: 'Abschluss',
      school: 'Hochschule/Schule, Ort',
      languages: 'Sprachen',
      languagesValue: 'Sprache (Niveau)',
      it: 'EDV',
      itValue: 'Programme und Kenntnisse',
    },
    invoice: {
      name: 'Rechnung',
      description: 'Rechnung auf der Geschäftsbrief-Geometrie: Positionstabelle mit Summenzeilen, Zahlungsziel, Bankverbindung.',
      number: 'Nummer',
      invoiceNo: 'Rechnungs-Nr.: ',
      customerNo: 'Kunden-Nr.: ',
      subject: 'Rechnung Nr. ',
      intro: 'wir stellen Ihnen folgende Leistungen in Rechnung:',
      colPos: 'Pos.',
      colDescription: 'Beschreibung',
      colQty: 'Menge',
      colUnitPrice: 'Einzelpreis',
      colAmount: 'Betrag',
      itemDesc: 'Leistung oder Artikel',
      qty: 'Anzahl',
      amount: '0,00 €',
      subtotal: 'Zwischensumme (netto)',
      vat: 'zzgl. 19 % USt.',
      total: 'Gesamtbetrag',
      paymentBefore: 'Zahlbar ohne Abzug bis zum ',
      paymentAfter: ' auf das unten genannte Konto.',
      bankLabel: 'Bankverbindung: ',
      bank: 'Bank',
      ibanLabel: 'IBAN: ',
      iban: 'IBAN',
      bicLabel: 'BIC: ',
      bic: 'BIC',
      vatIdLabel: 'USt-IdNr.: ',
    },
    minutes: {
      name: 'Besprechungsprotokoll',
      description: 'Protokoll mit Kopftabelle (Datum, Ort, Teilnehmer) und nummerierten TOPs mit Beschluss- und Aufgabenzeile.',
      title: 'Protokoll: ',
      topic: 'Thema der Besprechung',
      dateLabel: 'Datum',
      place: 'Ort',
      time: 'Beginn / Ende',
      timeValue: 'Uhrzeit von – bis',
      participants: 'Teilnehmer',
      participantNames: 'Namen',
      recorder: 'Protokollführung',
      recorderName: 'Name',
      item: 'TOP',
      itemTopic: 'Thema',
      discussion: 'Ergebnis der Diskussion',
      decision: 'Beschluss: ',
      decisionText: 'Beschlusstext',
      action: 'Aufgabe: ',
      actionTask: 'Aufgabe',
      actionOwner: ' — verantwortlich: ',
      ownerName: 'Name',
      actionDue: ', bis ',
      dueDate: 'Datum',
      next: 'Nächster Termin: ',
    },
  },
  lineNumbers: {
    title: 'Zeilennummerierung',
    off: 'Keine',
    continuous: 'Fortlaufend',
    perPage: 'Jede Seite neu beginnen',
    interval: 'Jede n-te Zeile nummerieren',
    countEmpty: 'Leerzeilen mitzählen',
  },
  revisions: {
    record: 'Änderungen verfolgen',
    display: 'Anzeige für Überprüfung',
    displayAll: 'Alle Markups',
    displayAllHint: 'Jede Änderung und jeder Kommentar',
    displaySimple: 'Einfaches Markup',
    displaySimpleHint: 'Endtext mit Änderungsbalken',
    displayNone: 'Kein Markup',
    displayNoneHint: 'Der Text, wie er sich liest',
    displayOriginal: 'Original',
    displayOriginalHint: 'Der Text vor den Änderungen',
    placeBalloons: 'In Sprechblasen',
    placePane: 'In der Leiste',
    prev: 'Vorherige',
    next: 'Nächste',
    printMarkup: 'Markup drucken',
    printMarkupHint: 'Randbalken und Kommentarliste mitdrucken',
    recordHint: 'Jede Bearbeitung als Änderung aufzeichnen',
    accept: 'Annehmen',
    acceptHint: 'Die Änderung an der Einfügemarke annehmen',
    reject: 'Verwerfen',
    rejectHint: 'Die Änderung an der Einfügemarke verwerfen',
    acceptAll: 'Alle annehmen',
    rejectAll: 'Alle verwerfen',
    pane: 'Änderungen',
    showPane: 'Änderungen anzeigen',
    showPaneHint: 'Liste der aufgezeichneten Änderungen anzeigen',
    empty: 'Keine aufgezeichneten Änderungen.',
    unknownAuthor: 'Unbekannter Autor',
    inserted: 'Eingefügt',
    deleted: 'Gelöscht',
  },
  notesDialog: {
    title: 'Fuß- und Endnoten',
    tabFootnotes: 'Fußnoten',
    tabEndnotes: 'Endnoten',
    numbering: 'Nummerierung',
    format: 'Format',
    startAt: 'Beginnen bei',
    restart: 'Nummerierung neu beginnen',
    restartDocument: 'Fortlaufend',
    restartPage: 'Pro Seite',
    restartChapter: 'Pro Abschnitt',
    position: 'Position',
    positionPage: 'Seitenende',
    positionDocument: 'Dokumentende',
    affix: 'Vor / nach der Nummer',
    prefix: 'Davor',
    suffix: 'Danach',
    styles: 'Vorlagen',
    bodyStyle: 'Notentext',
    citationStyle: 'Notenanker',
    separator: 'Trennlinie',
    sepWidth: 'Länge (% der Textbreite)',
    sepWeight: 'Dicke (pt)',
    sepAbove: 'Abstand oben (cm)',
    sepBelow: 'Abstand unten (cm)',
    sepAlign: 'Ausrichtung',
    sepColor: 'Farbe',
    reset: 'Auf LibreOffice-Standard zurücksetzen',
  },
  ribbon: {
    tabs: {
      file: 'Datei',
      home: 'Start',
      insert: 'Einfügen',
      layout: 'Layout',
      references: 'Verweise',
      review: 'Überprüfen',
      view: 'Ansicht',
      tableDesign: 'Tabellenentwurf',
      tableLayout: 'Tabellenlayout',
      pictureFormat: 'Bildformat',
      shapeFormat: 'Formformat',
    },
    saveAs: 'Speichern unter…',
    saveAsHint: 'In den Download-Einstellungen des Browsers aktivieren: „Immer nachfragen, wo Dateien gespeichert werden sollen“.',
    collapse: 'Menüband minimieren',
    expand: 'Menüband erweitern',
    export: 'Exportieren',
    find: 'Suchen',
    replace: 'Ersetzen',
    groups: {
      clipboard: 'Zwischenablage',
      font: 'Schriftart',
      paragraph: 'Absatz',
      pages: 'Seiten',
      tables: 'Tabellen',
      illustrations: 'Illustrationen',
      links: 'Links',
      headerFooter: 'Kopf- und Fußzeile',
      text: 'Text',
      symbols: 'Symbole',
      pageSetup: 'Seite einrichten',
      toc: 'Inhaltsverzeichnis',
      notes: 'Fußnoten',
      citations: 'Zitate und Literaturverzeichnis',
      index: 'Index',
      revisions: 'Nachverfolgung',
      markup: 'Markup',
      proofing: 'Rechtschreibung',
      language: 'Sprache',
      show: 'Anzeigen',
      debug: 'Debug',
      tableDecor: 'Schattierung & Rahmen',
      tableOptions: 'Tabellenformatoptionen',
      rowsColumns: 'Zeilen & Spalten',
      merge: 'Zusammenführen',
      cellSize: 'Zellengröße',
      tableData: 'Daten',
      arrange: 'Anordnen',
      shapeStyles: 'Formenarten',
      accessibility: 'Barrierefreiheit',
      captions: 'Beschriftungen',
      editing: 'Bearbeiten',
    },
    case: {
      title: 'Groß-/Kleinschreibung',
      none: 'Wie eingegeben',
      upper: 'GROSSBUCHSTABEN',
      lower: 'kleinbuchstaben',
      capitalize: 'Ersten Buchstaben Groß',
      smallCaps: 'Kapitälchen',
    },
    lineStyles: {
      solid: 'Einfach',
      double: 'Doppelt',
      dotted: 'Gepunktet',
      dashed: 'Gestrichelt',
      wavy: 'Wellenlinie',
    },
    pageBreak: 'Seitenumbruch',
    picture: 'Bild',
    textBox: 'Textfeld',
    crossRef: 'Querverweis',
    pageNumber: 'Seitenzahl',
    equation: 'Formel',
    header: 'Kopfzeile',
    footer: 'Fußzeile',
    hfOptions: 'Optionen',
    link: 'Link',
    bookmark: 'Textmarke',
    table: 'Tabelle',
    dateTime: 'Datum & Uhrzeit',
    symbol: 'Symbol',
    margins: 'Seitenränder',
    size: 'Größe',
    breaks: 'Umbrüche',
    insertAbove: 'Darüber einfügen',
    insertBelow: 'Darunter einfügen',
    insertLeft: 'Links einfügen',
    insertRight: 'Rechts einfügen',
    sectionBreak: 'Abschnittsumbruch',
    startsOnOdd: 'Auf rechter Seite beginnen',
    startsOnEven: 'Auf linker Seite beginnen',
    startsOnHint: 'Eine Leerseite einschieben, wenn der Abschnitt sonst auf der anderen Seite beginnt',
    sectionBreakHint: 'Beginnt eine eigene Kopf- und Fußzeile',
    indentLeft: 'Einzug links',
    indentRight: 'Einzug rechts',
    spaceBefore: 'Abstand davor',
    spaceAfter: 'Abstand danach',
    marginPresets: {
      normal: 'Normal',
      narrow: 'Schmal',
      wide: 'Breit',
    },
    toc: 'Inhaltsverzeichnis',
    tocOptions: 'Optionen',
    tocMaxLevel: 'Tiefste angezeigte Ebene',
    tocPageNumbers: 'Seitenzahlen anzeigen',
    insertCaption: 'Beschriftung',
    indexes: { toc: 'Inhaltsverzeichnis', figures: 'Abbildungsverzeichnis', tables: 'Tabellenverzeichnis', alphabetical: 'Stichwortverzeichnis', bibliography: 'Literaturverzeichnis' },
    indexEntry: 'Stichwort',
    indexEntryPrompt: 'Stichwort — das Wort, das im Verzeichnis steht:',
    indexEntryNeedsSelection: 'Markiere das Wort, das ins Stichwortverzeichnis soll',
    thisSection: 'Dieser Abschnitt',
    likeDocument: 'Wie das Dokument',
    citation: 'Literaturangabe',
    noteOptions: 'Optionen',
    autoCorrect: 'AutoKorrektur',
    hyphenation: 'Silbentrennung',
    repeatHeaderRow: 'Kopfzeile wiederholen',
    repeatHeaderRowHint: 'Die erste Zeile oben auf jeder Seite wiederholen',
    pageNumberFormat: 'Seitenzahlen',
    pageNumberStart: 'Beginnen bei',
    pageNumberContinue: 'Vom vorherigen Abschnitt fortsetzen',
    lineNumbers: 'Zeilennummern',
    pageDecor: 'Seitengestaltung',
    foldMarks: 'Falzmarken',
    foldMarksHint: 'Falz- und Lochmarken am linken Rand (DIN 5008)',
    hyphenationHint: 'Automatisch trennen',
    debugDump: 'Dump',
    cellMargins: 'Zellenränder',
    altText: 'Alternativtext',
    altTextHint: 'Das Bild beschreiben',
    chrome: {
      title: 'Symbolleisten-Stil',
      ribbon: 'Klassisch',
      ribbonHint: 'Mit Registerkarten, wie in Word',
      modern: 'Modern (beta)',
      modernHint: 'Eine schwebende Symbolleiste',
    },
  },
  app: {
    documentName: 'Dokumentname',
    untitled: 'Unbenanntes Dokument',
    startTyping: 'Hier tippen…',
    newDocument: 'Neues Dokument',
    open: 'Öffnen…',
    save: 'Speichern',
    saveExport: 'Speichern / Exportieren',
    unsavedChanges: 'Nicht gespeichert',
    odt: 'ODT',
    openDocument: 'OpenDocument',
    exporting: 'Wird exportiert…',
    rasterPdf: 'Raster-PDF',
    rasterHint: 'Exakte Kopie des Editors',
    vectorPdf: 'Vektor-PDF (Beta)',
    vectorHint: 'Scharf, aber Probleme mit Tabellen',
    template: 'Vorlage',
    templateHint: 'wie das Dokument: .ott oder .dotx — öffnet sich als neues Dokument',
    recentFiles: 'Zuletzt verwendet',
    clearRecentFiles: 'Liste leeren',
    print: 'Drucken',
    tools: 'Werkzeuge',
    showExtraTools: 'Weitere Werkzeuge anzeigen',
    hideExtraTools: 'Weitere Werkzeuge ausblenden',
  },
  status: {
    pageOf: (current, total) => `Seite ${current} von ${total}`,
    words: (n) => `${n.toLocaleString('de-DE')} ${n === 1 ? 'Wort' : 'Wörter'}`,
    selectedOf: (sel, total) =>
      `${sel.toLocaleString('de-DE')} von ${total.toLocaleString('de-DE')} Wörtern`,
    num: (n) => n.toLocaleString('de-DE'),
    statistics: 'Statistik',
    selection: 'Auswahl',
    document: 'Dokument',
    wordsLabel: 'Wörter',
    charsWithSpaces: 'Zeichen (mit Leerzeichen)',
    charsNoSpaces: 'Zeichen (ohne Leerzeichen)',
    paragraphs: 'Absätze',
    pages: 'Seiten',
    zoomOut: 'Verkleinern',
    zoomIn: 'Vergrößern',
    zoom: 'Zoom',
    resetZoom: 'Auf 100 % zurücksetzen',
  },
  dialogs: {
    confirmNew: 'Neues Dokument beginnen? Nicht gespeicherte Änderungen gehen verloren.',
    confirmReplace: 'Beim Öffnen dieser Datei wird das aktuelle Dokument ersetzt. Fortfahren?',
    openedWithLimitations: (list) => `Mit Einschränkungen geöffnet:\n• ${list}`,
    couldNotOpen: 'Diese Datei konnte nicht geöffnet werden.',
    chooseFormat: 'In welchem Format speichern?',
    couldNotSave: 'Diese Datei konnte nicht gespeichert werden.',
    scriptBlocked: 'Ein Teil der App konnte nicht geladen werden — ein Skript-Blocker (uBlock Origin, NoScript, strenger Schutzmodus) verhindert das. Erlaube diese Seite und versuche es erneut.',
    couldNotExportPdf: 'PDF-Export nicht möglich.',
    recentUnavailable: (name) =>
      `„${name}“ konnte nicht erneut geöffnet werden — die Datei wurde offenbar verschoben oder gelöscht. Öffne sie stattdessen über den Dateidialog.`,
    recentDenied: (name) =>
      `Der Browser hat das Lesen von „${name}“ nicht erlaubt. Erlaube den Zugriff im Berechtigungsdialog, oder öffne die Datei über den Dateidialog.`,
    couldNotPrint: 'Drucken nicht möglich.',
    couldNotPrintPdf: 'Druck als PDF nicht möglich.',
    autosaveQuota:
      'Das Dokument ist zu groß, um automatisch gespeichert zu werden (Speicherlimit des Browsers erreicht). Das passiert meist bei großen eingebetteten Bildern. Speichere es als .odt-Datei, um deine Arbeit zu sichern.',
    openSnapshot: (when: string) =>
      `Das Dokument konnte nicht geladen werden. Stattdessen die vom Editor gesicherte Fassung von ${when} öffnen?`,
    documentNotLoaded:
      'Das zuletzt bearbeitete Dokument hat den Start des Editors blockiert. Trotzdem noch einmal laden?\n\nAbbrechen startet mit einem neuen, leeren Dokument; das alte bleibt im Browser-Speicher unter „edentext-doc-broken“ erhalten.',
    remoteImagesDropped: (n) =>
      `${n === 1 ? 'Ein Bild' : `${n} Bilder`} aus dem Web ${n === 1 ? 'konnte' : 'konnten'} nicht in das Dokument übernommen werden und ${n === 1 ? 'wurde' : 'wurden'} entfernt. Speichere das Bild als Datei und füge es von dort ein.`,
    picturesNotRestored: (n) =>
      `${n} ${n === 1 ? 'Bild konnte' : 'Bilder konnten'} nicht aus dem Browser-Speicher wiederhergestellt werden und ${n === 1 ? 'wird' : 'werden'} als defekt angezeigt. Öffne die gespeicherte Datei des Dokuments, um ${n === 1 ? 'es' : 'sie'} zurückzuholen.`,
  },
  importWarn: {
    imagesSkipped: 'Einige Bilder konnten nicht gelesen werden und wurden übersprungen',
    shapesRemoved: 'Nicht unterstützte Formen wurden entfernt',
    drawingsRemoved: 'Zeichnungen wurden entfernt',
    noDictionary: (tag) => `Für die Rechtschreibsprache „${tag}" ist kein Wörterbuch enthalten — die Rechtschreibprüfung wurde deaktiviert`,
    hfFlattened: 'Listen/Tabellen in Kopf- oder Fußzeilen wurden in Text umgewandelt',
    textBoxFlattened: 'In Tabellenzellen oder anderen Textfeldern verschachtelte Textfelder wurden aufgelöst',
    nestedTables: 'Verschachtelte Tabellen wurden in Absätze umgewandelt',
    comments: 'Kommentare wurden entfernt',
    missingFonts: (fonts) => `Diese Schriftarten sind nicht installiert und wurden durch Ersatzschriften dargestellt: ${fonts}`,
  },
  password: {
    menu: 'Mit Passwort schützen…',
    menuOn: 'Geschützt',
    setTitle: 'Mit Passwort schützen',
    askTitle: 'Dieses Dokument ist geschützt',
    password: 'Passwort',
    repeat: 'Wiederholen',
    mismatch: 'Die beiden Eingaben unterscheiden sich.',
    wrong: 'Falsches Passwort.',
    note: 'Schützt die gespeicherte Datei. Die Kopie, die der Editor im Browser hält, bleibt unverschlüsselt.',
    show: 'Passwort anzeigen',
    hide: 'Passwort verbergen',
    remove: 'Schutz entfernen',
    lost: 'Das Passwort bleibt über einen Reload nicht erhalten. Gib es für die Datei erneut ein, oder entferne den Schutz.',
  },
  importError: {
    odtArchive: 'Keine gültige .odt-Datei (Archiv konnte nicht gelesen werden).',
    odtContentMissing: 'Keine gültige .odt-Datei (content.xml fehlt).',
    odtNoBody: 'Kein Textdokument (kein office:text-Inhalt).',
    odtMalformed: 'Keine gültige .odt-Datei (fehlerhaftes XML).',
    docxMalformed: 'Keine gültige .docx-Datei (fehlerhaftes XML).',
    docxArchive: 'Keine gültige .docx-Datei (Archiv konnte nicht gelesen werden).',
    docxMissing: 'Keine gültige .docx-Datei (word/document.xml fehlt).',
    docxNoBody: 'Kein Word-Dokument (kein w:body).',
    encryptionUnsupported: 'Dieses Dokument benutzt ein Verschlüsselungsverfahren, das nicht unterstützt wird.',
  },
};

export default de;
