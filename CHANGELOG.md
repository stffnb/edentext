...
- A table of contents is **regenerated** on load, never read from the field's cached rows — so an index a producer built with other settings comes back with every heading in the document, dot leaders and live page numbers. The one switch worth reading is Word's `TOC \n` / an ODF entry template with no `<text:index-entry-page-number/>`: an index that deliberately shows no page numbers is a layout, not a stale cache
