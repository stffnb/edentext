...
- Sort a table's rows (LibreOffice's Table ▸ Sort, Word's Layout ▸ Sort): by any column, ascending or descending, optionally keeping the first row as a header. The type is detected per cell rather than chosen — a column of numbers sorts numerically, anything else by the document language's collation — where both dialogs make you pick; and only the first of their three sort keys is offered
