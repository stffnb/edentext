...
- Bibliography: neither the entry (ODF `text:bibliography-mark`, Word's `CITATION` field over a source in its own custom-XML part) nor the generated list. The alphabetical index above is the same machinery, so this is a matter of the entry's fields and a source list, not of new plumbing
